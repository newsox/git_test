# -*- coding: utf-8 -*-
# @Time    : 2025/3/31 19:20
# @Author  : xu xinwen
# @FileName: validation_ltoa_difference_0330.py
# @Software: PyCharm
...
with ExitStack() as stack:
    ...
    # 在循环前初始化交互模式和图对象
    plt.ion()
    fig, ((ax1, ax2), (ax3, ax4), (ax5, ax6), (ax7, ax8),  (ax9, ax10)) = plt.subplots(5, 2, figsize=(18, 18))  # 创建2个子图
    ...
    ax3.set_ylabel(r'SIF ($\mathrm{mW \cdot m^{-2} \cdot sr^{-1} \cdot nm^{-1}}$)', fontsize=10)
    line_sif, = ax3.plot([], [], 'c--', label='SIF_TOC(Ltoa_difference/T_up)')
    line_sif_with_t, = ax3.plot([], [], 'b--', label='SIF_TOA(Ltoa_difference)')
    line_sif_original, = ax3.plot([],[], 'r--', label= 'SIF_TOC(SCOPE_original)')
    # SIF子图的图例和网格
    ...
    ax5.set_ylabel(r'SIF_TOC_Difference ($\mathrm{mW \cdot m^{-2} \cdot sr^{-1} \cdot nm^{-1}}$)', fontsize=10)
    line_toc, = ax5.plot([], [], 'y--', label='SIF_TOC_difference(original-algorithm)')
    # SIF子图的图例和网格
    ...
    ax6.set_ylabel(r'SIF_TOA_Difference ($\mathrm{mW \cdot m^{-2} \cdot sr^{-1} \cdot nm^{-1}}$)', fontsize=10)
    line_toa, = ax6.plot([], [], 'k--', label='SIF_TOA_difference(original*t-Ltoa_diifference)')
    # SIF子图的图例和网格
    ...
    line_t_algorithm, = ax2.plot([], [], 'r--', label='T(t/(1-s*ρ))')
    # T_up子图的图例和网格
    ...
    ax7.set_ylabel(r'T(100%)', fontsize=10)
    line_t1, = ax7.plot([], [], 'r--', label='T(t/(1-s*ρ))')
    line_t2, = ax7.plot([], [], 'g--', label='T_up')
    line_t3, = ax7.plot([], [], color='orange', linestyle='--', label = 'T')
    # T_up子图的图例和网格
    ...
    ax8.set_ylabel(r'value(100%)', fontsize=10)
    line_t_dif, = ax8.plot([], [], 'k--', label='Tdif{t_up-(t/(1-s*ρ)}')
    # T_up子图的图例和网格
    ...
    # （9）---- 配置sif子图 ----
    ax9.set_xlabel('Wavelength (nm)', labelpad=0.4)
    ax9.set_ylabel(r'SIF ($\mathrm{mW \cdot m^{-2} \cdot sr^{-1} \cdot nm^{-1}}$)', fontsize=10)
    line_sif_validation, = ax9.plot([], [], 'g--', label="SIF_TOA(Ltoa_difference)")
    line_sif_t_model, = ax9.plot([], [], 'b--', label="SIF_TOA(SIF_SCOPE*T_model)")
    line_sif_o, = ax9.plot([], [], 'r--', label="SIF_TOC(original)")
    # T_up子图的图例和网格
    ...
    ax10.set_ylabel(r'SIF_diff ($\mathrm{mW \cdot m^{-2} \cdot sr^{-1} \cdot nm^{-1}}$)', fontsize=10)
    line_sif_diff, = ax10.plot([], [], 'k--', label='SIFdiff')
    # T_up子图的图例和网格
    ...
    for i in range(l):
        ...
        ax5.set_ylim(np.min(line_toc_data) - 0.2,
        ...
        ax6.set_ylim(np.min(line_toa_data) - 0.2,
        ...
        line_sif_validation.set_data(wl, validation[i,:])
        ...
        ax10.set_ylim(np.min(sif_dif) - 0.1,
                      np.max(sif_dif) + 0.1)
